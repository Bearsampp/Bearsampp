...
FOR %%x IN ("~BEARSAMPP_WIN_PATH~\bin\php\php7.4.33\pear\pear") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=~BEARSAMPP_WIN_PATH~\bin\php\php7.4.33\pear\pear"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=~BEARSAMPP_WIN_PATH~\bin\php\php7.4.33\pear"
IF "%PHP_PEAR_PHP_BIN%"=="" SET "PHP_PEAR_PHP_BIN=~BEARSAMPP_WIN_PATH~\bin\php\php7.4.33\php.exe"
