...
class Util
{
     ...
     *
     * Automatically selects the most appropriate unit (GB, MB, KB, bytes),
     * unless a specific unit is forced via the $unit parameter.
     *
     * @param  int    $size  The file size in bytes.
     * @param  string $unit  Optional forced unit ('GB', 'MB', 'KB', or '').
     *                       When empty, the function auto‑selects the best unit.
     *
     * @return string        The formatted file size (e.g., "1.25GB", "512KB", "900 bytes").
     */
    public static function humanFileSize(int $size, string $unit = ''): string
    {
        ...
        // If a unit is forced, use it directly
        if ($unit !== '' && isset($units[$unit])) {
            return number_format($size / $units[$unit], 2) . $unit;
        }
        ...
        return $unit === ''
            ...
            : number_format($size / $units[$unit], 2) . $unit;
    }
