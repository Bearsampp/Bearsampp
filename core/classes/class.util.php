...
class Util
{
    ...
    public static function clearFolder($path, $exclude = array())
    {
        ...
        while ( false !== ($file = readdir( $handle )) ) {
            if ( $file == '.' || $file == '..' || in_array( $file, $exclude ) || $file == '.gitignore' ) {
                continue;
            }
            if ( is_dir( $path . '/' . $file ) ) {
    ...
    /**
     * Opens a file with the specified content using the default notepad application.
     *
     * This function creates a temporary directory and file, writes the provided content to the file,
     * and then opens it using the system's default notepad application. The file and directory names
     * are generated using a random string to ensure uniqueness.
     *
     * @param   string  $caption  The filename to be used, which will also serve as the window title in the notepad.
