...
class TplAppMongodb
{
    ...
    public static function process()
    {
        ...
        return TplApp::getMenuEnable($bearsamppLang->getValue(Lang::MONGODB), self::MENU, get_called_class(), $bearsamppBins->getMongodb()->isEnable());
    }

    public static function getMenuMongodb()
    {
        global $bearsamppBs, $bearsamppConfig, $bearsamppBins, $bearsamppLang, $bearsamppTools;
        $resultItems = $resultActions = '';

        $isEnabled = $bearsamppBins->getMongodb()->isEnable();
        ...
        if ($isEnabled) {
            ...
            // Conf
            $resultItems .= TplAestan::getItemNotepad(basename($bearsamppBins->getMongodb()->getConf()), $bearsamppBins->getMongodb()->getConf()) . PHP_EOL;

            // Logs
            $resultItems .= TplAestan::getItemNotepad(basename($bearsamppBins->getMongodb()->getErrorLog()), $bearsamppBins->getMongodb()->getErrorLog()) . PHP_EOL;
        }
    ...
    public static function getMenuMongodbVersions()
    {
        ...
        foreach ($bearsamppBins->getMongodb()->getVersionList() as $version) {
            $tplSwitchMongodbVersion = TplApp::getActionMulti(
                ...
                array($version, $version == $bearsamppBins->getMongodb()->getVersion() ? TplAestan::GLYPH_CHECK : ''),
                false, get_called_class()
            );
    ...
    public static function getActionEnableMongodb($enable)
    {
        ...
        return TplApp::getActionRun(Action::ENABLE, array($bearsamppBins->getMongodb()->getName(), $enable)) . PHP_EOL .
    ...
    public static function getActionSwitchMongodbVersion($version)
    {
        ...
        return TplApp::getActionRun(Action::SWITCH_VERSION, array($bearsamppBins->getMongodb()->getName(), $version)) . PHP_EOL .
    ...
    public static function getMenuMongodbService()
    {
        ...
        $isInstalled = $bearsamppBins->getMongodb()->getService()->isInstalled();

        $result = TplAestan::getItemActionServiceStart($bearsamppBins->getMongodb()->getService()->getName()) . PHP_EOL .
            TplAestan::getItemActionServiceStop($bearsamppBins->getMongodb()->getService()->getName()) . PHP_EOL .
            TplAestan::getItemActionServiceRestart($bearsamppBins->getMongodb()->getService()->getName()) . PHP_EOL .
            TplAestan::getItemSeparator() . PHP_EOL .
            TplApp::getActionRun(
                Action::CHECK_PORT, array($bearsamppBins->getMongodb()->getName(), $bearsamppBins->getMongodb()->getPort()),
                array(sprintf($bearsamppLang->getValue(Lang::MENU_CHECK_PORT), $bearsamppBins->getMongodb()->getPort()), TplAestan::GLYPH_LIGHT)
            ) . PHP_EOL .
    ...
    public static function getMenuMongodbDebug()
    {
        ...
        return TplApp::getActionRun(
            ...
            array($bearsamppLang->getValue(Lang::DEBUG_MONGODB_VERSION), TplAestan::GLYPH_DEBUG)
        ) . PHP_EOL;
    }
    ...
    public static function getActionChangeMongodbPort()
    {
        ...
        return TplApp::getActionRun(Action::CHANGE_PORT, array($bearsamppBins->getMongodb()->getName())) . PHP_EOL .
            TplAppReload::getActionReload();
    }
