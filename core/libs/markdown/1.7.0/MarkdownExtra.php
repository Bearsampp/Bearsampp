...
class MarkdownExtra extends \Michelf\Markdown {
	 ...
	 * lists, and tables. That's because we still want to wrap <div>s around
	 ...
	 * @param  boolean $wrap_in_p Whether paragraphs should be wrapped in <div> tags
	...
	protected function formParagraphs($text, $wrap_in_p = true) {
		...
		// Wrap <div> tags and unhashify HTML blocks
	...
	protected function appendFootnotes($text) {
		...
		if (!empty($this->footnotes_ordered)) {
			...
			while (!empty($this->footnotes_ordered)) {
				...
				if (preg_match('{</div>$}', $footnote)) {
					$footnote = substr($footnote, 0, -4) . "&#160;$backlink</div>";
				} else {
					$footnote .= "\n\n<div>$backlink</div>";
				}
