...
if ($bearsamppBins->getMailhog()->checkPort($smtpPort)) {
    if ($bearsamppBins->getMailhog()->checkPort($smtpPort)) {
        $result['checkport'] .= '<span style="float:right;font-size:12px" class="badge text-bg-success">' . sprintf($textServiceStarted, $smtpPort) . '</span>';
    } else {
...
$result['versions'] .= '<span style="float:right;font-size:12px;margin-left:2px;" class="badge text-bg-primary">' . $bearsamppBins->getMailhog()->getVersion() . '</span>';
