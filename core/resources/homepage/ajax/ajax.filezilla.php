...
if ($bearsamppBins->getFilezilla()->isEnable()) {
    if ($bearsamppBins->getFilezilla()->checkPort($sslPort, true)) {
        $result['checkport'] .= '<span class="float-right ms-1 badge text-bg-success">' . sprintf($textServiceStarted, $sslPort) . ' (SSL)</span>';
    } else {
        $result['checkport'] .= '<span class="float-right ms-1 badge text-bg-danger">' . $textServiceStopped . ' (SSL)</span>';
    }
...
$result['versions'] .= '<span class="float-right ms-1 badge text-bg-primary">' . $bearsamppBins->getFilezilla()->getVersion() . '</span>';
