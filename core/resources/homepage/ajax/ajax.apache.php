...
if ($bearsamppBins->getApache()->isEnable()) {
    if ($bearsamppBins->getApache()->checkPort($sslPort, true)) {
        $result['checkport'] .= '<span style="float:right;font-size:12px;margin-left:2px;" class="badge text-bg-success">' . sprintf($textServiceStarted, $sslPort) . ' (SSL)</span>';
    } else {
        $result['checkport'] .= '<span style="float:right;font-size:12px;margin-left:2px;" class="badge text-bg-danger">' . $textServiceStopped . ' (SSL)</span>';
    }
...
$result['versions'] .= '<span style="float:right;font-size:12px;margin-left:2px;" class="badge text-bg-primary">' . $bearsamppBins->getApache()->getVersion() . '</span>';

// Modules count
$modules = count($bearsamppBins->getApache()->getModules());
$modulesLoaded = count($bearsamppBins->getApache()->getModulesLoaded());
$result['modulescount'] .= '<span style="float:right;font-size:12px" class="badge text-bg-primary">' . $modulesLoaded . ' / ' . $modules . '</span>';

// Aliases count
$result['aliasescount'] .= '<span style="float:right;font-size:12px" class="badge text-bg-primary">' . count($bearsamppBins->getApache()->getAlias()) . '</span>';

// Vhosts count
$result['vhostscount'] .= '<span style="float:right;font-size:12px" class="badge text-bg-primary">' . count($bearsamppBins->getApache()->getVhosts()) . '</span>';
